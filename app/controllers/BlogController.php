...
class BlogController extends BaseController {
	...
	{
//        $mongodb = LMongo::connection();
//        $user = $mongodb->users->findOne(array("_id"=>1));

        $users = LMongo::collection('users')
            ->whereNin('_id', array(1, 2, 3))->get();

        foreach($users AS $user){
            var_dump($user['username']);
        }

//        $users = LMongo::collection('users')->where('username','newduminhtamupdate')->first();
//var_dump($users['username']);
//
//        LMongo::collection('users')
//            ->where('_id', 1)
//            ->update(array('username' => "newduminhtamupdate"));
